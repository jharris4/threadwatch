...
| `otbr` ot-ctl state via host:port: no_ssh | there is no ssh client on this host: nothing was tried against the border router | native: install openssh-client; Docker: rebuild the image (it installs one) and mount the key (docs/DOCKER.md, "Border router inventory") |
...
| `web` nothing answers on port N (of HOST) | the review pages are not up where `[web] bind` and `port` say | `systemctl status threadwatch-web`; `[web]` in config.toml |
