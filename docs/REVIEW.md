...
| `visitor_returned` ... `visitor_left` | one row spanning the visit: *4f2a… visited for 17s (visit 3)*, on the day of the visit (*visiting (not yet over)* until it is filed); a `device_quiet` an earlier run announced for the same address folds into it, and an open starvation or signal drop for the address ends with the visit |
