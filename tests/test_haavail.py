...
HA device map and the state poll, and the episode rules; a device's own
hold and mute are inventory fields (tests/test_names.py), read here
through the inventory by address."""
...
class TrackerTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.holds, self.mutes = {}, set()      # what the inventory would say, by (lower-case) address
        outer = self
        ...
        class Names:
            ...
            def hold_s(self, addr):
                return outer.holds.get(addr)

            def muted(self, addr):
                return addr in outer.mutes
        self.names = Names()
    ...
    def _tracker(self, state=True):
        ...
        return Tracker(self.cfg, self.d / "ha-availability.json" if state else None,
    ...
    def test_per_device_hold_and_mute_come_from_the_inventory(self):
        self.holds[ADDRS[0]] = 7200.0
        self.mutes.update({ADDRS[1], ADDRS[2]})
        tr = self._tracker()
        self._poll(tr, T0)
...
class RecorderAvailabilityTest(unittest.TestCase):
    ...
    def test_the_inventory_hold_applies_to_the_ha_check_through_the_pipeline(self):
        (self.d / "devices.json").write_text(json.dumps([dict(ENTRIES[0], hold_s=120), ENTRIES[1]]))
        from datetime import datetime, timezone
        pipe = self._pipe()
        now = 1_800_000_000.0
        self._cycle(pipe, now)
        self.states[0].update(state="unavailable",
                              last_changed=datetime.fromtimestamp(now + 30, timezone.utc).isoformat())
        pipe._next_haavail = 0.0
        self._cycle(pipe, now + 60)                                     # [ha_availability] hold_s is 0 here ...
        self.assertEqual([r for r in pipe.events.records if r["event"] == "ha_unavailable"], [])
        pipe._next_haavail = 0.0
        self._cycle(pipe, now + 200)                                    # ... the entry's own 120 s is what counts
        evs = [r for r in pipe.events.records if r["event"] == "ha_unavailable"]
        self.assertEqual([(e["name"], e["hold_s"], e["muted"], e["severity"]) for e in evs],
                         [("Front Path Motion", 120.0, False, "warning")])

    def test_the_state_files_travel_with_a_snapshot(self):
